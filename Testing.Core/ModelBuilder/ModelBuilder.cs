...
public sealed class ModelBuilder<T> where T : class
{
    ...
    {
        return _propertiesAssignedUsingSet.TryGetValue(propertyName, out var setValue)
            ? setValue
            : _propertiesAssignedUsingFor.TryGetValue(propertyName, out var forValueDelegate)
